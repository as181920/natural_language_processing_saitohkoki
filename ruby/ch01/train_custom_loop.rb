...
max_epoch.times.map do |epoch|
  ...
  max_iters.times.map do |iters|
    ...
    # 定期输出学习过程
    next unless ((iters + 1) % 10).zero?

    avg_loss = total_loss / loss_count
    print(format("| epoch %d |  iter %d / %d | loss %.2f\n", epoch + 1, iters + 1, max_iters, avg_loss)) # rubocop:disable Style/FormatStringToken
...
# 绘制决策边界
h = 0.001
x_min = x[0..-1, 0].min - 0.1
x_max = x[0..-1, 0].max + 0.1
y_min = x[0..-1, 1].min - 0.1
y_max = x[0..-1, 1].max + 0.1
